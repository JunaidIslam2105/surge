...
| `surge resume <id>`         | Resumes a paused or errored download by ID/prefix.                                     | `--all`                                                                                             | Retries the download using its saved state.                             |
...
Failed downloads remain in the list after an error, including after restarting Surge, with their error message available in `surge ls <id>` and the TUI. Use `surge resume <id>` to retry or `surge refresh <id> <url>` to replace a stale source URL.
